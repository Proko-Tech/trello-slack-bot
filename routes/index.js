...
router.post('/card', async function(req, res, next){
  if(req.body.action.display.translationKey === 'action_move_card_from_list_to_list'){
    const text = '@channel '+req.body.action.memberCreator.fullName+' has moved the card '+req.body.action.data.card.name+' from '+req.body.action.data.listBefore.name +' to '+req.body.action.data.listAfter.name+'. Check it out at: https://trello.com/c/'+req.body.action.data.card.shortLink;
